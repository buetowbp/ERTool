...
public class DraggableObject {
	...
	public boolean isFrontMost(ArrayList<DraggableObject> draggables) {
		for (int i = ERToolView.objects.size() - 1; i > -1; i--) {
			for (DraggableObject d2 : draggables) {
				if (ERToolView.objects.get(i).equals(d2))
					return false;

			}
			if (ERToolView.objects.get(i).equals(this))
				return true;
		}
	...
	public boolean isInFrontOf(DraggableObject d) {
		for (int i = ERToolView.objects.size() - 1; i > -1; i--) {

			if (ERToolView.objects.get(i).equals(this))
				return true;

			else if (ERToolView.objects.get(i).equals(d))
				return false;
		}
		return true; // should never get here
	}
	...
	public void delete() {
		// should be implemented and overridden in each type (Entity, Attribute,
		// Relationship)
	}
