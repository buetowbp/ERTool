...
public class Entity extends DraggableObject implements MouseListener {
	...
	public Entity(JViewport parent, ERScriptEntity link, String text,
			Boolean isWeak) {
		super();
		this.mLink = link;
		this.parent = parent;
		...
		this.color = Constants.colors[ERToolView.entities.size()
	...
	public void draw(Graphics g) {
		...
		if (this.attributes.size() != 0) {
			...
			for (Attribute a : this.attributes) {
				if (this.isInFrontOf(a)) {
					// If entity is in front of object, line should be drawn
					// from the other object
				} else {
					g.drawLine(x + width / 2, y + height / 2, a.x + width / 2,
							a.y + height / 2);
				}
			}
		...
		if (this.relationships.size() != 0) {
			for (Relationship r : this.relationships) {
				if (this.isInFrontOf(r)) {
					// If entity is in front of object, line should be drawn
					// from the other object
				} else {
					g.drawLine(x + width / 2, y + height / 2, r.x + width / 2,
							r.y + height / 2);
					r.drawConstraints(this, g,
							(x + width / 2 + r.x + width / 2) / 2, (y + height
									/ 2 + r.y + height / 2) / 2);
				}
			}
		}
		...
		if (this.isWeak) {
			...
			g.setColor(Color.GRAY); // color isn't dynamic; value must be
									// manually changed with canvas color
